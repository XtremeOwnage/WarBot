...
{
    public static class HustleExpedition
    {
        /// <summary>
        ...
        private static async Task sendHustleEventMessage(GuildLogic gLogic, HustleGuildChannelEventLogic eventConfig, string Message, bool IsFinalMessage = false)
        {
            ...
            if (await PermissionHelper.TestBotPermissionAsync(ch, ChannelPermission.SendMessages))
            {
                ...
                DeleteMessageJob.Enqueue(msg, eventConfig, IsFinalMessage);
            ...
            StringBuilder sb = new StringBuilder()
                ...
                .AppendLine($"You are receiving this error, because I do not have the proper permissions to send hustle expedition notification to channel {ch.Name}.")
        ...
        public static async Task EventStarted(GuildLogic cfg, HustleGuildChannelEventLogic warCfg)
        {
            ...
            if (!string.IsNullOrEmpty(Message))
                await sendHustleEventMessage(cfg, warCfg, Message, true);
        }
        public static async Task EventEnded(GuildLogic cfg, HustleGuildChannelEventLogic warCfg)
        {
            //Guild has elected out for this notification.
            if (!warCfg.Enabled)
                return;
